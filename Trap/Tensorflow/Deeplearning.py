...
class MLE:
    ...
    def Loglikelihood(self, data, mu, sigma, weight):
        if self._Distribution == 'normal':
            ...
            PDF = self.LognormalPdf;

        Result = 0;
        for i in range(self._NumComponent):
            Result += weight[i] * PDF(data, mu[i], sigma[i]);
    ...
    def Training(self, data):
        ...
        InitialMu, InitialStd, InitialWeight = MixtureModel.InitializeVariable(data, 
            self._Distribution, self._NumComponent);
        Cost_Eval = [];
