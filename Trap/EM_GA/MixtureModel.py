...
def GaussianMixture(data, numComponent):
    ...
    Iteration = 100;
    Loglikelihood = [0] * Iteration;

    for iter in range(Iteration):
        SumDenominator = 0;
        for k in range(numComponent):
            SumDenominator += Weight[k] * sas.norm.pdf(data, Mu[k], Sigma[k]);
        for k in range(numComponent):
            Q[:, k] = Weight[k] * sas.norm.pdf(data, Mu[k], Sigma[k]) / SumDenominator;
        ...
        LogTemp = 0;
        for k in range(numComponent):
            LogTemp += np.sum(np.log(Weight[k] * sas.norm.pdf(data, Mu[k], Sigma[k])));
...
def LognormalMixture(data, numComponent):
    ...
    return Lognormal_Mean(Mu, Sigma), Lognormal_Variance(Mu, Sigma), Lognormal_Skewness(Sigma), \
        Weight, Loglikelihood;

def Loglikelihood(data, distribution, numComponent, mu, sigma, weight):
    ...
    Loglikelihood = 0;
    for k in range(numComponent):
        Temp = weight[k] * PDF(data, mu[k], sigma[k]);
        Temp[Temp < 1e-20] = 1e-20;
